...
void parse_data_fun(std_msgs::String msg)
{
	...
	rece_str = rece_str + redundancy_string_last + msg.data;
...
void serial_receive_callback(const ros::TimerEvent&)//,std_msgs::Bool emg
{
	...
	else if(!ser1.available() && (ros::Time::now()-last_request) > ros::Duration(3.0))
	{
...
int main(int argc,char** argv)
{
	...
	ros::Timer timer = nh.createTimer(ros::Duration(0.002),serial_receive_callback);
	//circle frequence
	ros::Rate rate(300);
	//emg.data = false;
	while (ros::ok())
